...
void plotSkulls(cv::Mat& frame, LightController& lc) {

    for (int i=0; i<48; i++) {
      Node node = lc.getLight(i);
      Loc loc = lc.getLightLoc(node.skull, node.eye, node.light);
      cv::Point point = cv::Point(loc.x + 50, loc.y + 50);
      cv::Scalar color = cv::Scalar(node.color.b, node.color.g, node.color.r);
      circle(frame, point, 3, color, 4);
    }
